...
def computing_metrics(G, title, path):
    ...
    plt.savefig(path+'_figure_node_degrees.png')
    print('Figure saved!')
    print()

    avg_lengths_list = avg_lengths(G)
    print('Average path length:', np.mean(avg_lengths_list))
    fig, ax = plt.subplots()
    plt.hist(avg_lengths_list)
    plt.xlabel('Average time to reach other stops (in minutes)')
    plt.ylabel('Number of stops')
    plt.title('Average path length - ' + title)
    plt.savefig(path+'_figure_avg_path_length.png')
    print('Figure saved!')
    print()

    data = avg_nb_connections(G)
    print('Average number of connections:', np.mean(data))
    ...
    plt.savefig(path+'_figure_avg_nb_connections.png')
    print('Figure saved!')
    print()
